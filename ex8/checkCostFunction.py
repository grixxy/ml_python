...
# Zap out most entries
    Y = X_t.dot(Theta_t.T)
    Y[np.random.randint(Y.shape) > 0.5] = 0
    R = np.zeros(Y.shape)
    R[Y != 0] = 1

# Run Gradient Checking
    X = np.random.rand(X_t.shape)
    Theta = np.random.rand(Theta_t.shape)
    num_users = Y.shape[1]
...
#Cost func reference
    ...
    costFunc = functools.partial(cofiCostFunc, params = params, Y = Y, R = R, num_users = num_users, num_movies = num_movies, num_features = num_features, lambda_ = 0)
